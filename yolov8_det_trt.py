...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        cv2.rectangle(img, c1, c2, color1, -1, cv2.LINE_AA)  # fille
        print(label,end=' ')
        cv2.putText(
            img,
            label,
...
if __name__ == "__main__":
    ...
    engine_file_path = "./build/yolov8n.engine"
    ctypes.CDLL(PLUGIN_LIBRARY)
    # 初始化 TensorRT 推理类
    yolov8_trt = YoLov8TRT(engine_file_path)

    # 打开摄像头
    cap = cv2.VideoCapture(0)  # 使用摄像头(设备索引 0)
    if not cap.isOpened():
    ...
    while True:
        ...
        # 调用 TensorRT 推理
        frame_batch = [frame]  # 将单帧包装成批处理接口
        batch_image_raw, inference_time = yolov8_trt.infer(frame_batch)
        #fps
        frame_count += 1
        ...
        if elapsed_time>1:
            ...
            start_time=time.time()
        # 显示推理结果
        result_frame = batch_image_raw[0]
